 ...
 * @author CJC
 * @date 2024/6/20
 * @description twitter的snowflake算法 -- java实现协议格式： 0 - 41位时间戳 - 5位数据中心标识 - 5位机器标识 - 12位序列号
 */
@Slf4j
public class SnowflakeIdGenerator implements IdentifierGenerator {
    ...
    public synchronized Number nextId(Object entity) {
        ...
        if (currStmp < lastStamp) {
            log.error("Clock moved backwards. Refusing to generate id for {} milliseconds", lastStamp - currStmp);
            throw new AppException(GlobalResultCode.ID_GENERATE_ERROR);
        }
