...
public class StringUtils {
    ...
    public static boolean isAnyBlank(final CharSequence... css) {
        ...
        return Stream.of(css).anyMatch(jodd.util.StringUtil::isBlank);
    }
    ...
    public static boolean isNoneBlank(final CharSequence... css) {
        ...
        return Stream.of(css).allMatch(jodd.util.StringUtil::isNotBlank);
    }
    ...
    public static String filterEmptyString(String str) {
        return CharSequenceUtil.isEmptyOrUndefined(str) ? "" : str;
    }
     ...
     * @return 对象
     */
    public static String ternaryOperator(Object obj1, Object obj2) {
        if (ObjectUtil.isNotEmpty(obj1)) {
            return obj1.toString();
        }
        return ObjectUtils.isEmpty(obj2) ? null : obj2.toString();
    ...
    public static String format(String strPattern, String placeHolder, Object... argArray) {
        int count = countPlaceholders(strPattern, placeHolder);
        if (count == 0) {
            return strPattern;
        }
        if (ObjectUtils.isEmpty(argArray)) {
            argArray = new Object[count];
            Arrays.fill(argArray, "");
            return cn.hutool.core.text.StrFormatter.formatWith(strPattern, placeHolder, argArray);
        }
        if (argArray.length < count) {
            Object[] newArgArray = new Object[count];
            System.arraycopy(argArray, 0, newArgArray, 0, argArray.length);
            Arrays.fill(newArgArray, argArray.length, count, "");
            return cn.hutool.core.text.StrFormatter.formatWith(strPattern, placeHolder, newArgArray);
        }
        return cn.hutool.core.text.StrFormatter.formatWith(strPattern, placeHolder, argArray);
    }
    ...
    /**
     * 字符串转换为List
     * 例如： 将字符串  ["name","description"] 转为 List<String>
     *
     * @param object 字符串
     * @return List<String>
     */
    public static List<String> convertStringToList(Object object) {
        if (ObjectUtils.isEmpty(object)) {
            return Lists.newArrayList();
        }
        return JsonUtil.readValue(object.toString(), List.class);
    }
     ...
     * @return String
     */
    public static String convertListToSqlInCondition(List<?> values) {
     ...
     * @return String
     */
    public static <T, R> String convertListToString(List<T> values, Function<T, R> function) {
    ...
    public static String generateRandomPassword(int length) {
        ...
        // 确保每个字符类别至少出现一次
        passwordChars.add(UPPER.charAt(secureRandom.nextInt(UPPER.length())));
        passwordChars.add(LOWER.charAt(secureRandom.nextInt(LOWER.length())));
        ...
        // 生成剩余随机字符
        String remaining = RandomUtil.randomString(allChars, length - 4);
        ...
        passwordChars.forEach(sb::append);
        return sb.toString();

    }

    public static String convertDbColumn(String input) {
    ...
    /**
     * 获取最后一个点之后的内容，并将首字母小写
     *
     * @param api 字符串
     * @param toLower 是否将首字母小写
     * @return 字符串
     */
    public static String getContentAfterLastDot(String api, boolean toLower) {
        String afterDot = api;
        int lastDotIndex = api.lastIndexOf(StringPool.DOT);
        if (lastDotIndex != -1 && lastDotIndex < api.length() - 1) {
            afterDot = api.substring(lastDotIndex + 1);
        }
        String first = afterDot.substring(0, 1);
        if (toLower) {
            first = first.toLowerCase();
        }
        return first + afterDot.substring(1);
    }
     ...
     * @return 字符串
     */
    public static String getContentAfterLastDot(String api) {
        return getContentAfterLastDot(api, false);
    }
}
